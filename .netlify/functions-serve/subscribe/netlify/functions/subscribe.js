...
var import_mailchimp_marketing = __toESM(require_src2(), 1);
var MAILCHIMP_API_KEY = process.env.MAILCHIMP_API_KEY;
var MAILCHIMP_AUDIENCE_ID = process.env.MAILCHIMP_AUDIENCE_ID;
if (!MAILCHIMP_API_KEY || !MAILCHIMP_AUDIENCE_ID) {
  console.error("Missing Mailchimp environment variables.");
}
var DATACENTER = MAILCHIMP_API_KEY?.split("-")[1];
...
var handler = async (event) => {
  ...
  try {
    ...
    await import_mailchimp_marketing.default.lists.addListMember(MAILCHIMP_AUDIENCE_ID, {
      email_address: email,
      status: "subscribed",
      merge_fields: {
        FNAME: first || "",
